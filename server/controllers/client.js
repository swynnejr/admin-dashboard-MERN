...
// Mongoose Docs:  Model.find() https://mongoosejs.com/docs/api/model.html#model_Model-find
...
export const getGeography = async (req, res) => {
  try {
    ...
    // Nivo Charts: Choropleth https://nivo.rocks/choropleth/
    const mappedLocations = users.reduce((acc, { country }) => {
      // Use country-iso-2-to-3 library to format for Nivo
      const countryISO3 = getCountryIso3(country);
      // Talley the users per country
      if (!acc[countryISO3]) {
        acc[countryISO3] = 0;
      }
      acc[countryISO3]++;
